...
#define MAX_BUF_SIZE    1024
	...
	while (i < len) {
		...
		LIST_FOR_EACH_ENTRY_SAFE(event_list_cur, event_list_next,
		                         &sprd_data->events_list, link) {
...
_sprd_drm_vblank_event (int fd, tdm_sprd_data *sprd_data_p, void *hw_event_data)
{
	...
	tdm_sprd_vblank_data *vblank_data = (tdm_sprd_vblank_data * )((
	                unsigned long) hw_vblank->user_data);
	tdm_sprd_output_data *output_data;
...
static void
_sprd_drm_flip_complete_event (int fd, tdm_sprd_data *sprd_data_p,
                               void *hw_event_data)
{
	TDM_DBG("FLIP EVENT");
}
