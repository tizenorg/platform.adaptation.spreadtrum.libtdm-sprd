...
tdm_sprd_pp_handler(int fd, tdm_sprd_data *sprd_data_p, void *hw_event_data_p)
{
	...
	struct drm_sprd_ipp_event *hw_ipp_p = (struct drm_sprd_ipp_event *)
	                                      hw_event_data_p;
	...
	TDM_DBG("pp_data(%p) index(%d, %d) prop_id(%u)", pp_data, hw_ipp_p->buf_id[0],
	...
	if ((pp_data->prop_id = _tdm_sprd_pp_set(pp_data,
	                        &pp_data->tasks_array[pp_data->current_step],
	                        pp_data->prop_id)) <= 0) {
		TDM_ERR("Can't setup next pp step");
		return;
	}
	_tdm_sprd_pp_queue(pp_data, pp_data->prop_id,
	                   pp_data->task_buffers[pp_data->current_step].src,
...
sprd_pp_destroy(tdm_pp *pp)
{
	...
	if (pp_data->prop_id && pp_data->task_buffers[pp_data->current_step].src &&
	    pp_data->task_buffers[pp_data->current_step].dst) {
		_tdm_sprd_pp_queue(pp_data, pp_data->prop_id,
		                   pp_data->task_buffers[pp_data->current_step].src,
...
_sprd_pp_make_roadmap(tdm_sprd_pp_data *pp_data)
{
	...
	for (i = 1; i < max_size; i++) {
		...
		pp_data->temp_buffer[i - 1] = tbm_surface_internal_create_with_flags(
		...
		tbm_surface_map(pp_data->temp_buffer[i - 1], TBM_SURF_OPTION_READ,
		                &surface_info);
		...
		if (tbm_surface_internal_get_num_planes(pp_data->info.dst_config.format) > 1) {
			...
			TDM_DBG("BPP = %u", (tbm_surface_internal_get_bpp(
			                             pp_data->info.dst_config.format)));
			pp_data->tasks_array[i - 1].dst_config.size.h =
			        (surface_info.planes[0].stride << 3) / tbm_surface_internal_get_bpp(
			                pp_data->info.dst_config.format);
		}
		pp_data->tasks_array[i - 1].dst_config.size.v = tbm_surface_get_height(
		                        pp_data->temp_buffer[i - 1]);
		pp_data->tasks_array[i - 1].dst_config.pos.w =
		        width_leap[(((i - 1) < width_leap_size) ? (i - 1) : (width_leap_size - 1))];
		pp_data->tasks_array[i - 1].dst_config.pos.h =
		        height_leap[(((i - 1) < height_leap_size) ? (i - 1) : (height_leap_size - 1))];
		pp_data->tasks_array[i].transform = TDM_TRANSFORM_NORMAL;
		pp_data->tasks_array[i].src_config.format =
		        pp_data->tasks_array[i - 1].dst_config.format;
		pp_data->tasks_array[i].dst_config.format =
		        pp_data->tasks_array[i - 1].dst_config.format;
...
sprd_pp_commit(tdm_pp *pp)
{
	...
	if (pp_data->info_changed || pp_data->tasks_array_size > 1) {
		...
		if ((pp_data->prop_id = _tdm_sprd_pp_set(pp_data, &pp_data->tasks_array[0],
		                        pp_data->prop_id)) <= 0)
			return TDM_ERROR_OPERATION_FAILED;
